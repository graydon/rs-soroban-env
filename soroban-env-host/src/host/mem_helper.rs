use soroban_env_common::{
    xdr::{ScHostFnErrorCode, ScHostObjErrorCode, ScHostValErrorCode},
    RawVal, U32Val, Void,
};
...
use crate::{
    budget::CostType,
    host::VmSlice,
    host_object::{HostObjectType, MemHostObjectType},
    Host, HostError, Vm, VmCaller,
};
...
impl Host {
    ...
    fn memobj_clone_resize_and_copy_bytes_in<HOT: MemHostObjectType>(
        ...
        let obj_pos: u32 = obj_pos.into();
        let mut obj_new: Vec<u8> = self.visit_obj(obj, move |hv: &HOT| hv.metered_clone(&self.0.budget))?.into();
        let obj_end = obj_pos
            .checked_add(len)
