use crate::{impl_wrapper_tryfroms_and_tryfromvals, SymbolSmall};
use stellar_xdr::{ScStatus, ScStatusType};

use super::{Env, Object, Status, TryFromVal};
...
impl_wrapper_tryfroms_and_tryfromvals!(());
impl_wrapper_tryfroms_and_tryfromvals!(bool);
impl_wrapper_tryfroms_and_tryfromvals!(u32);
impl_wrapper_tryfroms_and_tryfromvals!(i32);
impl_wrapper_tryfroms_and_tryfromvals!(Status);
...
impl RawVal {
    ...
    #[inline(always)]
    pub(crate) const fn try_as_object(self) -> Option<Object> {
        if self.is_object() {
            Some(Object(self))
        } else {
            None
    ...
    pub const fn from_void() -> RawVal {
        unsafe { RawVal::from_body_and_tag(0, Tag::Void) }
    }

    #[inline(always)]
    pub const fn from_bool(b: bool) -> RawVal {
        let tag = if b { Tag::True } else { Tag::False };
        unsafe { RawVal::from_body_and_tag(0, tag) }
    }
...
impl RawVal {
    ...
    pub const VOID: RawVal = RawVal::from_void();
...
impl Debug for RawVal {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        match self.get_tag() {
            ...
            Tag::I32 => write!(f, "I32({})", (self.get_body() as u32) as i32),
            ...
            Tag::SymbolSmall => {
                unsafe { <SymbolSmall as RawValConvertible>::unchecked_from_val(*self) }.fmt(f)
            }
...
fn test_debug() {
    ...
    assert_eq!(
        format!("{:?}", SymbolSmall::from_str("hello")),
        "Symbol(hello)"
    );
    assert_eq!(
