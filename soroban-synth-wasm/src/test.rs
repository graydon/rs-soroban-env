...
fn test_synth_wasm() {
    ...
    fe.push(Symbol::from_small_str("somekey"));
    fe.local_set(s);
    ...
    fe.map_put(Operand::StackTop, s, RawVal::from_u32(123).to_raw());
...
fn call_local() {
    ...
    fe.push(Symbol::from_small_str("pass"));
    let (m0, f0) = fe.finish();
    // the caller
...
fn call_indirect() {
    ...
    let mut fe = me.func(Arity(0), 0);
    fe.push(Symbol::from_small_str("fail"));
...
fn br_table() {
    ...
    fe.end(); // 0
    fe.push(Symbol::from_small_str("a"));
    fe.ret();
    fe.end(); // 1
    fe.push(Symbol::from_small_str("b"));
    fe.ret();
    fe.end(); // 2
    fe.push(Symbol::from_small_str("c"));
    fe.ret();
    fe.end(); // 3
    fe.push(Symbol::from_small_str("d"));
    fe.ret();
    let bytes = fe.finish_and_export("test").finish();
    let printed = print_bytes(bytes).expect("wasmprinter");
    // run `UPDATE_EXPECT=true cargo test` to update this.
