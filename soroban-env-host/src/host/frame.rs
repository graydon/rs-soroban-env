...
impl Host {
        ...
        F: FnOnce() -> Result<Val, HostError>,
    {
        ...
        let rp = self.push_context(ctx)?;
        {
            ...
            if let Some(ctx) = self.try_borrow_context_stack()?.last() {
                let _ = self.with_vmcontext(|vmctx| {
                    self.call_any_lifecycle_hook(crate::host::TraceEvent::PushCtx(ctx), vmctx)
                })?;
            }
        }
        #[cfg(any(test, feature = "testutils"))]
        ...
        }
        {
            ...
            if let Some(ctx) = self.try_borrow_context_stack()?.last() {
                ...
                let _ = self.with_vmcontext(|vmctx| {
                    self.call_any_lifecycle_hook(crate::host::TraceEvent::PopCtx(&ctx, &res), vmctx)
                })?;
            }
        }
        if res.is_err() {
                ...
                return self.with_frame(Frame::TestContract(frame), || {
                    ...
                    match res {
                        ...
                        Err(panic_payload) => {
                            ...
                            // `VmContextEnv::escalate_error_to_panic`: fish the
