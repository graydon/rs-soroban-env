...
impl Debug for RawVal {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        match self.get_tag() {
            ...
            Tag::Error => {
                unsafe { <Error as RawValConvertible>::unchecked_from_val(*self) }.fmt(f)
            }
...
#[cfg(feature = "std")]
fn test_debug() {
    use super::{Object, Error, SymbolSmall};
    use crate::{
    ...
    assert_eq!(
        format!(
            "{:?}",
            Error::from_scerror(ScError{type_: ScErrorType::Value, code: ScErrorCode::InvalidInput})
        ),
        "Error(ScErrorType::Value, ScErrorCode::InvalidInput)"
    );
}
